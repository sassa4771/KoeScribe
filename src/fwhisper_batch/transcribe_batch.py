...
def transcribe_one(model: WhisperModel, audio_path: Path, out_dir: Path,
                   language: str, beam_size: int, use_vad: bool,
                   min_silence_ms: int, word_timestamps: bool) -> Dict[str, Any]:
    ...
    # Segments JSONL (with optional word timing)
    rows = []
    for s in seg_list:
    ...
    return {"text": text, "processing_time": proc_time, "language": info.language, "duration": info.duration}
...
def main():
    ...
    for idx, p in enumerate(tqdm(inputs, desc="🎧 Transcribing", unit="file"), start=1):
        ...
        result = transcribe_one(model, p, target_out, language, beam_size, use_vad, min_silence, args.word_timestamps)
